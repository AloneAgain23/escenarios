...
// Genera y devuelve el HTML completo directamente — sin redirects
...
function scenarioCard(sc) {
  ...
  <section class="scenario-card" style="--sc-color:${b.color};--sc-bg:${b.bg};">
    ...
    <div class="sc-grid">
      <div class="sc-col"><div class="sc-section-label">⚡ Impulsores</div><ul>${bulletList(sc.impulsores,'impulsor')}</ul></div>
      <div class="sc-col"><div class="sc-section-label">⚠ Riesgos</div><ul>${bulletList(sc.riesgos,'riesgo')}</ul></div>
      <div class="sc-col"><div class="sc-section-label">✦ Oportunidades</div><ul>${bulletList(sc.oportunidades,'oportunidad')}</ul></div>
    </div>
    <div class="sc-bottom">
      <div class="sc-implications"><div class="sc-section-label">▸ Implicancias</div><ul>${bulletList(sc.implicancias)}</ul></div>
      <div class="sc-signals"><div class="sc-section-label">📡 Señales</div><ul>${bulletList(sc.senales_monitoreo,'senal')}</ul></div>
    </div>
  </section>`;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const insumoSections = [
     ...
     .map(([label,arr]) => `<div class="insumo-group"><div class="insumo-label">${label}</div><ul>${arr.map(i=>`<li>${esc(i)}</li>`).join('')}</ul></div>`).join('');

    const scenarioCards = escenarios.map(sc => scenarioCard(sc)).join('');
...
<main class="main">
  ${pregunta_central ? `<div class="pregunta-box"><div class="pregunta-label">❓ Pregunta central</div><div class="pregunta-text">${esc(pregunta_central)}</div></div>` : ''}
  <div class="two-col">
    <div class="panel"><div class="panel-header">Insumos seleccionados</div><div class="panel-body">${insumoSections || '<p style="color:#999;font-size:.85rem;">No se registraron insumos.</p>'}</div></div>
    <div class="panel"><div class="panel-header">Lectura prospectiva</div><div class="panel-body"><p class="lectura-text">${esc(lectura) || '<em>Sin lectura registrada.</em>'}</p></div></div>
  </div>
...
<footer>CEPLAN — Centro Nacional de Planeamiento Estratégico &nbsp;|&nbsp; ${fecha} &nbsp;|&nbsp; Ejercicio exploratorio de demo</footer>
</body>
</html>`;

    // Devolver el HTML directamente — sin redirect, sin segundo request
