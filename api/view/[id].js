// api/view/[id].js
// Serves the HTML dashboard for a given session
...
function badges(tipo) {
  const map = {
    tendencial: { label: 'Tendencial', color: '#1A5FA8', bg: '#EAF1FB' },
    optimista:  { label: 'Optimista',  color: '#1A7A3C', bg: '#E8F5EC' },
    adverso:    { label: 'Adverso',    color: '#C8102E', bg: '#FDF0F2' },
  };
  return map[tipo] || { label: tipo, color: '#555', bg: '#f0f0f0' };
}

function bulletList(arr, cls = '') {
  if (!arr || !arr.length) return '<li class="empty-li">—</li>';
  return arr.map(i => `<li class="${cls}">${escHtml(i)}</li>`).join('');
}

function scenarioCard(sc, idx) {
  const b = badges(sc.tipo);
  const icons = { tendencial: '→', optimista: '↑', adverso: '↓' };
  const icon = icons[sc.tipo] || '•';
  return `
  <section class="scenario-card tipo-${sc.tipo}" style="--sc-color:${b.color};--sc-bg:${b.bg};" aria-label="Escenario ${idx+1}">
    <div class="sc-header">
      <span class="sc-icon">${icon}</span>
      <div class="sc-head-text">
        <span class="sc-tipo-badge" style="background:${b.bg};color:${b.color};">${escHtml(b.label)}</span>
        <h2 class="sc-nombre">${escHtml(sc.nombre || '—')}</h2>
      </div>
    </div>
    <div class="sc-narrativa">${escHtml(sc.narrativa || '')}</div>
    <div class="sc-grid">
      <div class="sc-col">
...
export default async function handler(req, res) {
  ...
  const raw = await kv.get(`scenario:${id}`).catch(() => null);
...
<body><div class="box"><h1>⚠ Reporte no encontrado</h1>
<p>Este reporte no existe o ha expirado.<br>Genera uno nuevo desde ChatGPT.</p></div></body></html>`);
  }
  ...
  const createdStr = created_at ? new Date(created_at).toLocaleString('es-PE') : '—';
...
<head>
  ...
  <title>${escHtml(tema)} — Escenarios CEPLAN</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header-brand span { display: block; font-size: .6rem; font-weight: 400; opacity: .75; letter-spacing: .12em; }
    .header-badge {
    ...
    .hero-eyebrow {
      ...
      letter-spacing: .2em; text-transform: uppercase; color: var(--rojo); margin-bottom: .5rem;
    }
    .hero-title {
      font-family: 'Playfair Display', serif;
      font-size: clamp(1.7rem, 4vw, 2.8rem);
      font-weight: 700; color: var(--texto); line-height: 1.15; margin-bottom: .75rem;
    }
    ...
    .pregunta-label {
      ...
      text-transform: uppercase; letter-spacing: .14em; color: var(--rojo); margin-bottom: .3rem;
    }
    .pregunta-text {
      font-family: 'Playfair Display', serif;
      font-size: 1.05rem; font-style: italic; color: var(--texto); line-height: 1.5;
    }

    /* TWO-COL: INSUMOS + LECTURA */
    ...
    .lectura-text {
      font-size: .9rem; line-height: 1.75; color: var(--texto);
      font-family: 'Source Sans 3', sans-serif;
    }
    ...
    .sc-header {
      ...
      padding: 1.2rem 1.5rem; background: var(--sc-bg, #f9f9f9);
    ...
    .sc-tipo-badge {
      ...
      text-transform: uppercase; letter-spacing: .12em;
      padding: .15rem .5rem; border-radius: 3px; display: inline-block; width: fit-content;
    }
    .sc-nombre {
      font-family: 'Playfair Display', serif;
      font-size: 1.15rem; font-weight: 700; color: var(--texto); line-height: 1.2;
    }

    .sc-narrativa {
      padding: 1.2rem 1.5rem;
      font-size: .92rem; line-height: 1.75; color: var(--texto);
    ...
    .sc-col li, .sc-implications li, .sc-signals li {
      ...
      border-bottom: 1px solid var(--gris2); color: var(--texto); line-height: 1.4;
    }
    .sc-col li:last-child, .sc-implications li:last-child, .sc-signals li:last-child {
      border-bottom: none;
    }
    .impulsor::before  { content: '⚡ '; font-size: .65rem; }
    .riesgo::before    { content: '⚠ '; font-size: .65rem; }
    .oportunidad::before { content: '✦ '; font-size: .65rem; color: var(--verde); }
    ...
    .cierre-header {
      background: var(--rojo-dark); color: #fff;
      padding: .7rem 1.5rem;
      font-size: .7rem; font-weight: 700;
      text-transform: uppercase; letter-spacing: .12em;
    }
    ...
    .cierre-label {
      font-size: .63rem; font-weight: 700;
      text-transform: uppercase; letter-spacing: .12em; color: var(--muted); margin-bottom: .5rem;
    }
    ...
    /* FOOTER */
    footer {
      background: var(--rojo-dark); color: rgba(255,255,255,.7);
...
<div class="hero">
  <div class="hero-inner">
    ...
    <h1 class="hero-title">${escHtml(tema)}</h1>
    <div class="meta-pills">
      ${sector       ? `<span class="meta-pill"><strong>Sector:</strong> ${escHtml(sector)}</span>` : ''}
      ${territorio   ? `<span class="meta-pill"><strong>Territorio:</strong> ${escHtml(territorio)}</span>` : ''}
      ${horizonte    ? `<span class="meta-pill"><strong>Horizonte:</strong> ${escHtml(horizonte)}</span>` : ''}
      ${meta.model   ? `<span class="meta-pill"><strong>Modelo:</strong> ${escHtml(meta.model)}</span>` : ''}
    </div>
  </div>
</div>
...
<main class="main">
  ...
  <div class="pregunta-box">
    ...
    <div class="pregunta-text">${escHtml(pregunta_central)}</div>
  </div>` : ''}
  ...
  <div class="two-col">
    ...
    <div class="panel">
      ...
      <div class="panel-body"><p class="lectura-text">${escHtml(lectura) || '<em>Sin lectura registrada.</em>'}</p></div>
    </div>
  </div>

  <div class="section-heading">
    <span>◈</span> Escenarios narrativos
  </div>
  ...
  ${(cierre.hallazgos || cierre.advertencia || cierre.descargo) ? `
  <div class="cierre-box">
    <div class="cierre-header">Cierre ejecutivo</div>
    <div class="cierre-body">
      ${cierre.hallazgos && cierre.hallazgos.length ? `
      <div>
        <div class="cierre-label">Principales hallazgos</div>
        <ul>${cierre.hallazgos.map(h => `<li>${escHtml(h)}</li>`).join('')}</ul>
      </div>` : ''}
      ${cierre.advertencia ? `
      <div>
        <div class="cierre-label">Advertencia metodológica</div>
        <p style="font-size:.87rem;color:#444;line-height:1.6;">${escHtml(cierre.advertencia)}</p>
      </div>` : ''}
      ${cierre.descargo ? `<p class="cierre-descargo">${escHtml(cierre.descargo)}</p>` : ''}
    </div>
  </div>` : ''}
...
<footer>
  ...
  Generado: ${createdStr} &nbsp;|&nbsp;
  Ejercicio exploratorio de demo · Válido 30 días
</footer>
