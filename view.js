// api/view.js
// Lee los datos desde el query param ?d= (base64url)
...
module.exports = async function handler(req, res) {
  ...
  if (!d) {
    ...
    return res.status(400).send('<h1>Parámetro faltante</h1>');
  }

  let session;
  try {
    session = JSON.parse(Buffer.from(d, 'base64url').toString('utf8'));
  } catch {
    res.setHeader('Content-Type','text/html; charset=utf-8');
...
<head>
  ...
  <style>
    :root {
      ...
      --gris2:#E8EAEC; --borde:#DDE1E6; --texto:#111827;
      --muted:#6B7280; --blanco:#FFFFFF;
    }
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .header-inner{max-width:1200px;margin:0 auto;padding:.85rem 2rem;display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;}
    .header-brand{font-weight:700;font-size:.95rem;text-transform:uppercase;letter-spacing:.06em;color:#fff;}
    .header-brand span{display:block;font-size:.6rem;font-weight:400;opacity:.75;letter-spacing:.12em;}
    .header-badge{background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.3);border-radius:4px;padding:.28rem .75rem;font-size:.7rem;font-weight:600;color:#fff;}
    .hero{background:var(--blanco);border-bottom:4px solid var(--rojo);padding:2.5rem 2rem 2rem;}
    .hero-inner{max-width:1200px;margin:0 auto;}
    .hero-eyebrow{font-size:.68rem;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:var(--rojo);margin-bottom:.5rem;}
    ...
    .sc-tipo-badge{font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.12em;padding:.15rem .5rem;border-radius:3px;display:inline-block;width:fit-content;}
    ...
    .sc-col{padding:1rem 1.2rem;border-right:1px solid var(--gris2);}
    ...
    @media(max-width:640px){.main{padding:1.5rem 1rem 3rem;}.hero{padding:1.5rem 1rem 1rem;}.header-inner{padding:.8rem 1rem;}}
  </style>
</head>
<body>
<header>
  <div class="header-inner">
    <div class="header-brand">Prospectiva Estratégica<span>CEPLAN — Centro Nacional de Planeamiento Estratégico</span></div>
...
<main class="main">
  ...
  <div class="two-col">
    ...
    <div class="panel">
      ...
      <div class="panel-body"><p class="lectura-text">${esc(lectura) || '<em>Sin lectura registrada.</em>'}</p></div>
    </div>
  </div>
  ...
  <div class="cierre-box">
    ...
    <div class="cierre-body">
      ${cierre.hallazgos?.length ? `<div><div class="cierre-label">Hallazgos</div><ul>${cierre.hallazgos.map(h=>`<li>${esc(h)}</li>`).join('')}</ul></div>` : ''}
      ${cierre.advertencia ? `<div><div class="cierre-label">Advertencia</div><p style="font-size:.87rem;color:#444;line-height:1.6;">${esc(cierre.advertencia)}</p></div>` : ''}
      ${cierre.descargo ? `<p class="cierre-descargo">${esc(cierre.descargo)}</p>` : ''}
    </div>
  </div>` : ''}
</main>
<footer>CEPLAN — Centro Nacional de Planeamiento Estratégico &nbsp;|&nbsp; Generado: ${createdStr} &nbsp;|&nbsp; Ejercicio exploratorio de demo</footer>
</body>
</html>`;
